...
module.exports = async function (db, game, player, context, limits) {
    ...
    if (game.gamesData[context.peerId].convData.isActive == true && game.gamesData[context.peerId].convData.gamemode == 'down7up' && context.messagePayload && context.messagePayload.action == 'd7u' && context.messagePayload.bet == 'up' && context.messagePayload.type == 'conversation') {
        if(player.playersData[context.senderId].banned == true) return
if(game.gamesData[context.peerId].convGame.bets[context.senderId]) {
			if(game.gamesData[context.peerId].convGame.bets[context.senderId].down || game.gamesData[context.peerId].convGame.bets[context.senderId].seven) {
			return context.send({
				message: `${player.playersData[context.senderId].mark}${player.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${player.playersData[context.senderId].name}]` : `${player.playersData[context.senderId].name}`}${player.playersData[context.senderId].mark}, вы уже поставили в этом раунде.`
			});
			}
		}
        ...
        if (scale <= 0) {
            _coin = await context.question({
                message: `${player.playersData[context.senderId].mark}${player.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${player.playersData[context.senderId].name}]` : `${player.playersData[context.senderId].name}`}${player.playersData[context.senderId].mark}, введи сумму ставки на выпадение числа больше 7:`
            })
        }
        if (scale > 0 && player.playersData[context.senderId].userData.globalSettings.allowInlineButtons == true) {
            _coin = await context.question({
                message: `${player.playersData[context.senderId].mark}${player.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${player.playersData[context.senderId].name}]` : `${player.playersData[context.senderId].name}`}${player.playersData[context.senderId].mark}, введи сумму ставки на выпадение числа больше 7 ИЛИ нажми на кнопку:`,
        ...
        for (i in game.gamesData) {
            if (game.gamesData[i].convGame.bets[context.senderId] && game.gamesData[i].convGame.bets[context.senderId].up && game.gamesData[i].convData.isActive == true && game.gamesData[i].convData.gamemode == 'down7up') {
                currentStavka += Math.floor(game.gamesData[i].convGame.bets[context.senderId].up)
            }
        }
        currentStavka += Number(_coin)
        if (Number(currentStavka > db.botSettings.gamesSettings.Down7Up.maxBets.up)) {
            return context.send({
                message: `${player.playersData[context.senderId].mark}${player.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${player.playersData[context.senderId].name}]` : `${player.playersData[context.senderId].name}`}${player.playersData[context.senderId].mark}, общая сумма ставок на выпадение числа больше 7 не должна превышать ${util.number_format(db.botSettings.gamesSettings.Down7Up.maxBets.up)}`
            })
        }
        ...
        if (!game.gamesData[context.peerId].convGame.bets[context.senderId].up) {
            game.gamesData[context.peerId].convGame.bets[context.senderId].up = 0
        }
        ...
        game.gamesData[context.peerId].convGame.bets[context.senderId].up += Math.floor(_coin)
        ...
        if (Number(player.playersData[context.senderId].balance) >= Number(_coin)) {
            ...
            return context.send(`${player.playersData[context.senderId].mark}${player.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${player.playersData[context.senderId].name}]` : `${player.playersData[context.senderId].name}`}${player.playersData[context.senderId].mark}, успешная ставка ${util.number_format(_coin)} коинов на выпадение числа больше 7`)
        }
        }
    }
}