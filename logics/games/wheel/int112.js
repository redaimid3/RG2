...
module.exports = async function (db, game, player, context, limits) {
    ...
    if (game.gamesData[context.peerId].convData.isActive == true && game.gamesData[context.peerId].convData.gamemode == 'wheel' && context.messagePayload && context.messagePayload.action == 'wheel' && context.messagePayload.bet == '1-12' && context.messagePayload.type == 'conversation') {
        ...
        if (scale > 0 && player.playersData[context.senderId].userData.globalSettings.allowInlineButtons == true) {
            _coin = await context.question({
                message: `${player.playersData[context.senderId].mark}${player.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${player.playersData[context.senderId].name}]` : `${player.playersData[context.senderId].name}`}${player.playersData[context.senderId].mark}, введи сумму ставки на промежуток 1-12 ИЛИ нажми на кнопку:`,
        ...
        for (i in game.gamesData) {
            if (game.gamesData[i].convGame.bets[context.senderId] && game.gamesData[i].convGame.bets[context.senderId].int112 && game.gamesData[i].convData.isActive == true && game.gamesData[i].convData.gamemode == 'wheel') {
        ...
        if (!game.gamesData[context.peerId].convGame.bets[context.senderId].int112) {
            game.gamesData[context.peerId].convGame.bets[context.senderId].int112 = 0
        }
        game.gamesData[context.peerId].convGame.bets[context.senderId].int112 += Math.floor(_coin)
