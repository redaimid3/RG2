...
module.exports = async function (db, game, player, context, limits) {
    ...
    if (game.gamesData[context.peerId].convData.isActive == true && game.gamesData[context.peerId].convData.gamemode == 'mini_wheel' && context.messagePayload && context.messagePayload.action == 'mini_wheel' && context.messagePayload.bet == '1-6' && context.messagePayload.type == 'conversation') {
    	if(player.playersData[context.senderId].banned == true) return
		if(game.gamesData[context.peerId].convGame.bets[context.senderId]) {
		if(game.gamesData[context.peerId].convGame.bets[context.senderId].int1936) return context.send({
				message: `${player.playersData[context.senderId].mark}${player.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${player.playersData[context.senderId].name}]` : `${player.playersData[context.senderId].name}`}${player.playersData[context.senderId].mark}, вы уже поставили на 7-12.`
			});
		}
        ...
        if (scale > 0 && player.playersData[context.senderId].userData.globalSettings.allowInlineButtons == true) {
            _coin = await context.question({
                message: `${player.playersData[context.senderId].mark}${player.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${player.playersData[context.senderId].name}]` : `${player.playersData[context.senderId].name}`}${player.playersData[context.senderId].mark}, введи сумму ставки на промежуток 1-6 ИЛИ нажми на кнопку:`,
        ...
        for (i in game.gamesData) {
            if (game.gamesData[i].convGame.bets[context.senderId] && game.gamesData[i].convGame.bets[context.senderId].int118 && game.gamesData[i].convData.isActive == true && game.gamesData[i].convData.gamemode == 'mini_wheel') {
                currentStavka += Math.floor(game.gamesData[i].convGame.bets[context.senderId].int118)
            }
        }
        currentStavka += Number(_coin)
        if (Number(currentStavka > db.botSettings.gamesSettings.mini_wheel.maxBets.intW)) {
            return context.send({
                message: `${player.playersData[context.senderId].mark}${player.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${player.playersData[context.senderId].name}]` : `${player.playersData[context.senderId].name}`}${player.playersData[context.senderId].mark}, общая сумма ставок промежутки не должна превышать ${util.number_format(db.botSettings.gamesSettings.mini_wheel.maxBets.intW)} коинов`
            })
        }
        ...
        if (!game.gamesData[context.peerId].convGame.bets[context.senderId].int16) {
            game.gamesData[context.peerId].convGame.bets[context.senderId].int16 = 0
        }
        game.gamesData[context.peerId].convGame.bets[context.senderId].int16 += Math.floor(_coin)
