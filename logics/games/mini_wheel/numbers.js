...
module.exports = async function (db, game, player, context, limits) {
    ...
    if (game.gamesData[context.peerId].convData.isActive == true && game.gamesData[context.peerId].convData.gamemode == 'mini_wheel' && context.messagePayload && context.messagePayload.action == 'mini_wheel' && context.messagePayload.bet == 'numbers' && context.messagePayload.type == 'conversation') {
    	if(player.playersData[context.senderId].banned == true) return
        let number = await context.question({
            message: `${player.playersData[context.senderId].mark}${player.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${player.playersData[context.senderId].name}]` : `${player.playersData[context.senderId].name}`}${player.playersData[context.senderId].mark}, введите число на которое желаете поставить:`
        })
        if(number.payload) return         
        if(isNaN(number.text)) { 
			return context.send(`Сначала выбери, на что ставить!`)
		}
        if(number.text > 12 || number.text < 1) return context.send({
            message: `${player.playersData[context.senderId].mark}${player.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${player.playersData[context.senderId].name}]` : `${player.playersData[context.senderId].name}`}${player.playersData[context.senderId].mark}, число должно находиться в диапазоне от 1 до 12.`
        })
        ...
        if (scale > 0 && player.playersData[context.senderId].userData.globalSettings.allowInlineButtons == true) {
            _coin = await context.question({
                message: `${player.playersData[context.senderId].mark}${player.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${player.playersData[context.senderId].name}]` : `${player.playersData[context.senderId].name}`}${player.playersData[context.senderId].mark}, введите сумму ставки на выпадение числа ${number.text}:`,
        ...
        for (i in game.gamesData) {
            if (game.gamesData[i].convGame.bets[context.senderId] && game.gamesData[i].convGame.bets[context.senderId] && game.gamesData[i].convData.isActive == true && game.gamesData[i].convData.gamemode == 'wheel') {
        ...
        if (Number(currentStavka > db.botSettings.gamesSettings.mini_wheel.maxBets.numbersOne)) {
            return context.send({
                message: `${player.playersData[context.senderId].mark}${player.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${player.playersData[context.senderId].name}]` : `${player.playersData[context.senderId].name}`}${player.playersData[context.senderId].mark}, общая сумма ставок на число ${number.text} не должна превышать ${util.number_format(db.botSettings.gamesSettings.mini_wheel.maxBets.numbersOne)} коинов.`
            })
        }
        let currentSum = 0
        for (i in game.gamesData) {
            if (game.gamesData[i].convGame.bets[context.senderId] && game.gamesData[i].convGame.bets[context.senderId] && game.gamesData[i].convData.isActive == true && game.gamesData[i].convData.gamemode == 'mini_wheel') {
        ...
        if (Number(currentSum > db.botSettings.gamesSettings.mini_wheel.maxBets.numberSum)) {
            return context.send({
                message: `${player.playersData[context.senderId].mark}${player.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${player.playersData[context.senderId].name}]` : `${player.playersData[context.senderId].name}`}${player.playersData[context.senderId].mark}, общая сумма ставок на числа не должна превышать ${util.number_format(db.botSettings.gamesSettings.mini_wheel.maxBets.numberSum)} коинов.`
            })
        }
        ...
        if (!game.gamesData[context.peerId].convGame.bets[context.senderId].numbers) {
            game.gamesData[context.peerId].convGame.bets[context.senderId].numbers = {}
        }
        if(!game.gamesData[context.peerId].convGame.bets[context.senderId].numbers[number.text]) {
            game.gamesData[context.peerId].convGame.bets[context.senderId].numbers[number.text] = {
                amount: 0,
                number: number.text
            }
        }
        game.gamesData[context.peerId].convGame.bets[context.senderId].numbers[number.text].amount += Math.floor(_coin)
        ...
        if (Number(player.playersData[context.senderId].balance) >= Number(_coin)) {
            ...
            return context.send(`${player.playersData[context.senderId].mark}${player.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${player.playersData[context.senderId].name}]` : `${player.playersData[context.senderId].name}`}${player.playersData[context.senderId].mark}, успешная ставка ${util.number_format(_coin)} коинов на выпадение числа ${number.text}`)
        }
        }
    }
}