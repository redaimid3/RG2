    ...
    if (game.gamesData[context.peerId].convData.isActive == true && game.gamesData[context.peerId].convData.gamemode == 'mini_wheel' && context.messagePayload && context.messagePayload.action == 'mini_wheel' && context.messagePayload.bet == 'noteven' && context.messagePayload.type == 'conversation') {
        ...
        if (scale > 0 && player.playersData[context.senderId].userData.globalSettings.allowInlineButtons == true) {
            _coin = await context.question({
                message: `${player.playersData[context.senderId].mark}${player.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${player.playersData[context.senderId].name}]` : `${player.playersData[context.senderId].name}`}${player.playersData[context.senderId].mark}, введи сумму ставки на нечетное ИЛИ нажми на кнопку:`,
        ...
        for (i in game.gamesData) {
            if (game.gamesData[i].convGame.bets[context.senderId] && game.gamesData[i].convGame.bets[context.senderId].noteven && game.gamesData[i].convData.isActive == true && game.gamesData[i].convData.gamemode == 'mini_wheel') {
                currentStavka += Math.floor(game.gamesData[i].convGame.bets[context.senderId].noteven)
            }
        }
        currentStavka += Number(_coin)
        if (Number(currentStavka > db.botSettings.gamesSettings.mini_wheel.maxBets.evenNoteven)) {
            return context.send({
                message: `${player.playersData[context.senderId].mark}${player.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${player.playersData[context.senderId].name}]` : `${player.playersData[context.senderId].name}`}${player.playersData[context.senderId].mark}, общая сумма ставок на нечетное не должна превышать ${util.number_format(db.botSettings.gamesSettings.mini_wheel.maxBets.evenNoteven)} коинов`
            })
        }
        ...
        if (!game.gamesData[context.peerId].convGame.bets[context.senderId].noteven) {
            game.gamesData[context.peerId].convGame.bets[context.senderId].noteven = 0
        }
        game.gamesData[context.peerId].convGame.bets[context.senderId].noteven += Math.floor(_coin)
